...
#define getc(fp) ((fp)->_cnt-- > 0 ? (*(fp)->_ptr++ & 0377) : _filbuf(fp))

#ifndef PUTCLBUF

#define putc(c, fp) ((fp)->_cnt-- > 0 ? (*(fp)->_ptr++ = (c)) : _flsbuf(c, fp))

#else

#define putc(c, fp) ((fp)->_cnt-- > 0 ? ((*(fp)->_ptr = (c)) == '\n' && ((fp)->_flag & _IOLBF) ? _flsbuf(*(fp)->_ptr, fp) : *(fp)->_ptr++) : _flsbuf(c, fp))
